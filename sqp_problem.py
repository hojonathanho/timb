...
#GRID_MIN = (-1., -1.)
#GRID_MAX = (1., 1.)
#PIXEL_AREA = (GRID_MAX[0]-GRID_MIN[0])/float(GRID_NX) * (GRID_MAX[1]-GRID_MIN[1])/float(GRID_NY)
WEIGHT_SCALE = 100
...
class TrackingProblem(object):
  def __init__(self): # TODO: put world bounds here
    ...
    self.u_names = make_varname_mat('u', GRID_SHAPE + (2,))
  ...
  def set_coeffs(self, flow_norm=None, flow_rigidity=None, obs=None, flow_agree=None):
    if flow_norm is not None:
      self.flow_norm_cost.coeff = WEIGHT_SCALE * PIXEL_AREA * flow_norm
    if flow_rigidity is not None:
      self.flow_rigidity_cost.coeff = WEIGHT_SCALE*PIXEL_AREA*flow_rigidity
    if obs is not None:
      self.obs_cost.coeff = WEIGHT_SCALE*sqrt(PIXEL_AREA)*obs
    if flow_agree is not None:
      self.flow_agree_cost.coeff = WEIGHT_SCALE*PIXEL_AREA*flow_agree
